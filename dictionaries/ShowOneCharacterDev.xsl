...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="index">
    <xsl:for-each select="//Lexicon/LexicalEntry">
      <xsl:if test="starts-with(./Lemma/FormRepresentation/feat[@att='scriptName' and @val='devanagari']//ancestor::FormRepresentation/feat[@att='citationForm']//@val, $char)">
        <!-- Insert link -->
        <xsl:element name="a">
          ...
          <xsl:attribute name="class">lexeme_dev</xsl:attribute>
          <xsl:call-template name="get">
            <xsl:with-param name="value" select="./Lemma/FormRepresentation/feat[@att='scriptName' and @val='devanagari']//ancestor::FormRepresentation/feat[@att='citationForm']//@val"/>
          </xsl:call-template>
          <br/>
        </xsl:element>
      </xsl:if>
      ...
      <dl>
        ...
        <!--<span class="national">
            ...
            <span class="vernacular2">
             ...
             <span class="vernacular2">
              (<xsl:value-of
                select="./Lemma/feat[@att='lexeme']//@val"
              />)
              <xsl:text>	</xsl:text>
            </span>
          </xsl:if>
