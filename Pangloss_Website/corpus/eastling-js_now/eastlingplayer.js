...
(function ($) {
    $.fn.eastlingplayer = function (options)
    {
        ...
        function to_DOM(annotations,metadata,url_images,url_audio,player){
            ...
            var div_alter = $('<div id="alternatives"></div>');
            var div_chercheurs = $('<div id="chercheurs"></div>');
            var div_langue = $('<div id="langue"></div>');
            var div_locuteurs = $('<div id="locuteurs"></div>');
            var div_sponsors = $('<div id="sponsors"></div>');
            var div_date = $('<div id="date"></div>');
            var div_crdo_id = $('<div id="crdo_id"></div>');
            var div_geo = $('<div id="geo"></div>');
            var div_location = $('<div id="location"></div>');
            var div_url_audio = $('<div id="url_audio"></div>');
            var div_url_image = $('<div id="url_image"></div>');
            ...
            $.each(metadata.contributors,function(){
                switch(this.code){
                    ...
                    case 'sponsor':sponsors.push(this.contributor);break; 

                }
            });

            $.each(metadata.title,function(index,value){
                if(index==0){
                    div_titre.html(value.title);
                }else{
                    alters.push(value.title);
                }
            });

            div_url_audio.html(url_audio);
            div_url_image.html(url_images.join((';')));
            div_alter.html(alters.join((';')));
            div_chercheurs.html(researchers.join((';')));
            div_sponsors.html(sponsors.join((';')));
            ...
            div_date.html(metadata.created);

            if(metadata.hasOwnProperty("spatial")){
                div_location.html(metadata.spatial[2].spatial);
                div_geo.html(metadata.spatial[1].spatial);
            }
            ...
            div_info.append(div_titre);
            div_info.append(div_alter);
            div_info.append(div_chercheurs).append(div_locuteurs).append(div_sponsors).append(div_date).append(div_location).append(div_geo).append(div_crdo_id);
            div_info.append(div_url_audio);
            div_info.append(div_url_image);
             ...
             if(words.length > 0){
                ...
                $.each(words,function(){
                   ...
                   this.areas[0].y2-=delta_y;  
                    ...
                    if(this.startPosition.length==0) div_word.addClass('noposition');
